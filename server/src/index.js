...
const decryptSettings = (row) => {
  try {
    ...
    console.error('Decryption failed, returning null settings:', err.message);
    return null;
  }
};
...
io.on('connection', (socket) => {
  ...
  socket.on('image_patch', (payload, callback) => {
    ...
    try {
      ...
      const decrypted = row ? decryptSettings(row) : null;
      const baseSettings = decrypted || { gallery: { images: [], imageClocks: {}, imageTombstones: {} } };
